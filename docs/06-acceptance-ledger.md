...
| ACC-M5-M55-004 | E2E | 前端 Playwright E2E 冒烟：注册→首页 4 人设卡片→进小雨会话→发消息收 AI 回复；CI docker job 内起 mock 栈后 `npx playwright test` | GitHub Actions push（run 34085706354） | success | 1 test passed（chromium），四 job 全绿 | frontend/e2e/smoke.spec.ts、frontend/playwright.config.ts、.github/workflows/ci.yml | ✅ | 2026-09-07 |
| ACC-M5-M55-005 | E2E | E2E 门禁首战即捕获真实前端 bug：**独立整页深链被弹回登录**——直接打开 `/register`（未登录）会落到 `/login`；根因 App.vue 挂载时机早于路由初始导航（未 `router.isReady()`），其重复的“无 token 跳登录”把深链弹走 | 修复：App.vue 挂载先 `await router.isReady()` 再按需加载会话列表，删除重复跳转（路由守卫已负责鉴权） | E2E 通过 | run 34085706354 success；修复后直开 /register 正常进入注册页（fda3237 部分修复 + 478a6bf 根治） | frontend/src/App.vue、frontend/e2e/smoke.spec.ts | ✅ | 2026-09-07 |
