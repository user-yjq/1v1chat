...
`01 → 02 → 03 → 04 → 05`；开发中每完成一步必须同步更新 `05 / 06 / 07`；发布/部署前看 `09`。
...
| 09 | 部署与发布手册 | 生产部署、回滚、发布 checklist（v0.5.0） | 每次发布/部署前更新 |
| 10 | 上线验收走查手册 | 部署试运行 + 真模型评测 + 压测大纲（scripts/walkthrough_live.py） | 真实环境走查时对照 |
...
| v0.6.0 | 上线前收尾（数据权/遗留清理） | 🚧 M5.1 ✅（账号级导出/删除）、M5.2 ✅（v1 引擎归档 `_legacy/engine_v1/` + 转发层保回滚，sqlite 148/PG 149 绿）；M5.3 ✅（前端“数据”弹窗）；M5.4 验收工具就绪（walkthrough_live.py + docs/10，待线上执行）；剩余遗留见 09 §7 |
